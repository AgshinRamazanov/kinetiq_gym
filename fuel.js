...
function renderFuel(){
  const targets=activeFuelTargets(),mode=fuelModes[fuelMode];document.querySelectorAll('[data-fuel-mode]').forEach(button=>button.classList.toggle('selected',button.dataset.fuelMode===fuelMode));document.getElementById('fuel-mode-note').textContent=mode.note;document.getElementById('nutrition-target-label').textContent=mode.label;document.getElementById('nutrition-calories').textContent=targets.calories.toLocaleString();['protein','carbs','fat'].forEach(key=>{document.getElementById(`nutrition-${key}`).textContent=`${targets[key]}g goal · 0g eaten`;document.getElementById(`nutrition-${key}-bar`).style.width='0%'});regeneratePlan();
}
